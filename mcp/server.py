...
def vps_status() -> dict:
    ...
    # Last commit
    vps_repo = os.environ.get("VPS_REPO_PATH", "")
    if vps_repo:
        _, commit = ssh_cmd(vps_ssh, f"cd {vps_repo} && git log --oneline -1 2>/dev/null")
    else:
        commit = "(VPS_REPO_PATH not configured)"
...
def dependency_scan(target: str) -> dict:
    ...
    """
    vps_repo_path = os.environ.get("VPS_REPO_PATH", "")
    search_dirs = [d for d in [vps_repo_path, str(Path.home() / ".claude")] if d]
    refs = []
...
def context_budget() -> dict:
    ...
    # CLAUDE.md
    vps_repo_path = os.environ.get("VPS_REPO_PATH", "")
    claude_md = Path(vps_repo_path) / "CLAUDE.md" if vps_repo_path else None
...
def github_changelog(repo_path: str = "", since: str = "", limit: int = 20) -> dict:
    ...
    Args:
        repo_path: local path to repo (default: VPS_REPO_PATH env var)
        since: git date filter (e.g. "2026-03-20", "1 week ago")
        limit: max commits to return
    """
    if not repo_path:
        repo_path = os.environ.get("VPS_REPO_PATH", "")
    if not repo_path:
        return {"error": "VPS_REPO_PATH not configured"}
...
def sync_status() -> dict:
    ...
    vps_repo_path = os.environ.get("VPS_REPO_PATH", "")
    vps_repo_name = os.path.basename(vps_repo_path) if vps_repo_path else ""
    repos = {}
    if vps_repo_name and vps_repo_path:
        repos[vps_repo_name] = Path(vps_repo_path)
    repos.update({
    ...
    if not ok:
        ...
        vps["reachable"] = True
        # Git HEAD comparison
        if vps_repo_path and vps_repo_name:
            _, vps_head = ssh_cmd(vps_ssh, f"cd {vps_repo_path} && git rev-parse --short HEAD 2>/dev/null")
            mac_head = github_mac.get(vps_repo_name, {}).get("head", "?")
        else:
            vps_head = "(VPS_REPO_PATH not configured)"
            mac_head = "?"
    ...
    # --- 4. Content pipeline ---
    content = {}
    drafts_dir = Path(vps_repo_path) / "content_drafts" if vps_repo_path else None
